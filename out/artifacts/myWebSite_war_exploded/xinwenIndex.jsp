...
<div class="coll_top">
    <div class="current clearfloat">
        ...
        <div class="locat"> <span>您当前的位置：</span><a href="">首页 &gt;&gt;</a><em>新闻公告</em> </div>
    </div>
</div>


            <div class="resu_list">
                    <ul class="clearfloat active">
                        <c:forEach items="${articleList}" var="item" varStatus="ss">
                        <%--<li>
                            <div class="pap_tit"> <a href="InformServlet?type=get&id=${item.id}"><em></em> ${item.title} </a> </div>
                            <div class="time_box"> <span class="time">${item.createtime}</span> </div>
                        </li>--%>
                            <li class="clearfloat"> <em></em><a href="ArticleServlet?type=get&id=${item.id}"> ${item.title} </a> <span>${item.createtime}</span> </li>
                        </c:forEach>
